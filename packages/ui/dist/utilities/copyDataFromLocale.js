    ...
    if (fieldAffectsData(field)) {
      switch (field.type) {
        case 'array':
          ...
          [null, undefined].includes(toLocaleData[field.name]) && field.name in fromLocaleData) {
        ...
        case 'blocks':
          ...
          if (field.name in toLocaleData) {
            toLocaleData[field.name].map((blockData, index) => {
              const block = req.payload.blocks[blockData.blockType] ?? (field.blockReferences ?? field.blocks).find(block => typeof block !== 'string' && block.slug === blockData.blockType);
              // Generate new IDs if the field is localized to prevent errors with relational DBs.
              if (fieldShouldBeLocalized({
                field,
                parentIsLocalized
              ...
              if (block?.fields?.length) {
                iterateFields(block?.fields, fromLocaleData[field.name][index], blockData, req, parentIsLocalized || field.localized);
              }
            });
          }
          break;
        ...
        case 'group':
          {
            if (field.name in toLocaleData && fromLocaleData?.[field.name] !== undefined) {
              iterateFields(field.fields, fromLocaleData[field.name], toLocaleData[field.name], req, parentIsLocalized || field.localized);
            }
            break;
          }
      }
          ...
          field.tabs.map(tab => {
            if (tabHasName(tab)) {
              if (tab.name in toLocaleData && fromLocaleData?.[tab.name] !== undefined) {
                iterateFields(tab.fields, fromLocaleData[tab.name], toLocaleData[tab.name], req, parentIsLocalized);
              }
            } else {
...
export const copyDataFromLocaleHandler = async args => {
  const {
    req
  } = args;
  try {
...
export const copyDataFromLocale = async args => {
  ...
  return globalSlug ? await payload.updateGlobal({
    slug: globalSlug,
    data: overrideData ? fromLocaleDataWithoutID : mergeData(fromLocaleDataWithoutID, toLocaleDataWithoutID, globals[globalSlug].config.fields, req, false),
    locale: toLocale,
    overrideAccess: false,
    req,
    user
    ...
    id: docID,
    collection: collectionSlug,
    data: overrideData ? fromLocaleDataWithoutID : mergeData(fromLocaleDataWithoutID, toLocaleDataWithoutID, collections[collectionSlug].config.fields, req, false),
    locale: toLocale,
    overrideAccess: false,
    req,
    user
